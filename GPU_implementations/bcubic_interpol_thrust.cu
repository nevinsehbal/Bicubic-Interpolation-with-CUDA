...
int main(void) {
    ...
    std::cout << "Time taken to interpolate image: " << milliseconds << " ms" << std::endl;
    ...
    milliseconds = 0;
    cudaEventElapsedTime(&milliseconds, start, stop);
    std::cout << "Time taken to copy data back to host: " << milliseconds << " ms" << std::endl;

    // Measure time to convert data to uint8_t using std::chrono
    ...
    std::cout << "Time taken to convert data to uint8_t: " << cpu_duration.count() << " ms" << std::endl;


    // Measure time to write image to disk using std::chrono
    cpu_start = std::chrono::high_resolution_clock::now();
    std::string output_image_path = input_image_path.substr(0, input_image_path.find_last_of(".")) + "_bicubic_interpolated_test1.png";
    ...
    std::cout << "Time taken to write image to disk: " << cpu_duration.count() << " ms" << std::endl;
