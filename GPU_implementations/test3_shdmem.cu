...
__global__ void bicubic_interpolation_kernel(
    ...
    if (x < original_width && y < original_height) {
        for (int c = 0; c < NUM_CHANNELS; ++c) {
            int global_idx = (y * original_width + x) * NUM_CHANNELS + c;
            int shared_idx = (ty * blockDim.x + tx) * NUM_CHANNELS + c;
            shared_image[shared_idx] = original_image[global_idx];
        }
    }
    __syncthreads();
            ...
            for (int j = 0; j < 4; ++j) {
                for (int i = 0; i < 4; ++i) {
                    ...
                    if (px < original_width && py < original_height) {
                        int shared_idx = ((py % blockDim.y) * blockDim.x + (px % blockDim.x)) * NUM_CHANNELS + c;
                        pixel_value = shared_image[shared_idx];
                    } else {
                        pixel_value = 0; // Out of bounds protection
                    }
...
int main(void) {
    ...
    size_t shared_memory_size = block_dim.x * block_dim.y * NUM_CHANNELS * sizeof(int);
    bicubic_interpolation_kernel<<<grid_dim, block_dim, shared_memory_size>>>(
    ...
    if (err != cudaSuccess) {
        std::cerr << "Error in kernel: " << cudaGetErrorString(err) << std::endl;
        return -1;
    }
    ...
    if (err != cudaSuccess) {
        std::cerr << "Error after kernel synchronization: " << cudaGetErrorString(err) << std::endl;
        return -1;
    }
    ...
    std::string output_image_path = input_image_path.substr(0, input_image_path.find_last_of(".")) + "_bicubic_interpolated_shdmem3.png";
